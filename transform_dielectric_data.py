...
def sim_kappa(W, k0, Delta_k, tau, rho):
    """
    Compute the complex dielectric function κ(ω) for a Debye dielectric with conductivity.


    Parameters:
        W (ndarray): Angular frequency array (rad/s).
        k0 (float): High-frequency dielectric constant.
        Delta_k (float): Dielectric relaxation strength (difference between low and high-frequency limits).
        tau (float): Debye relaxation time constant (seconds).
        rho (float): Resistivity of the dielectric medium (Ω·m).


    Returns:
        ndarray: Complex dielectric function κ(ω).
    """
    kappa = k0 + Delta_k / (1 + 1j * W * tau) - 1j / (rho * scc.epsilon_0 * W)
    return kappa
